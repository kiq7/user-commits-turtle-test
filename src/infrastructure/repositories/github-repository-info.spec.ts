...
describe('GitHub Repository Info', () => {
  const fakeDateUtils: any = {
    ...
    formatDateString: jest.fn().mockReturnValue('fake-date')
  }
  ...
  describe('getCommitsByUserAndDate', () => {
    it('should calls `listCommits` with correct params', async () => {
      const fakeClient: any = {
        repos: {
          listCommits: jest.fn()
      ...
      const repository = new GitHubRepositoryInfo(fakeClient, fakeDateUtils)
      await repository.getCommitsByUserAndDate('react', 'facebook', 'fake-author', '2020-10-10')

      expect(fakeDateUtils.formatDateToIso).toHaveBeenNthCalledWith(1, { date: '2020-10-10' })
      expect(fakeDateUtils.formatDateToIso).toHaveBeenNthCalledWith(2, { date: '2020-10-10', formatEndDayTime: true })
      expect(fakeClient.repos.listCommits).toHaveBeenCalledWith({
        owner: 'react',
        repo: 'facebook',
    ...
    it('should throws RepositoryNotFoundError when gh api returns 404', async () => {
      ...
      const repository = new GitHubRepositoryInfo(fakeClient, fakeDateUtils)
      await expect(repository.getCommitsByUserAndDate('react', 'facebook', 'fake-author', '2020-10-10')).rejects.toThrowError(RepositoryNotFoundError)
    })
  })
})
